...
fn classify(path: &Path) -> (EntryKind, u64, Option<SystemTime>, Option<PathBuf>) {
    let Ok(lmeta) = std::fs::symlink_metadata(path) else {
        return (EntryKind::Other, 0, None, None);
    };
    let modified = lmeta.modified().ok();
    if lmeta.is_symlink() {
        let target = std::fs::read_link(path).ok();
        return match std::fs::metadata(path) {
            Ok(tmeta) if tmeta.is_dir() => (EntryKind::SymlinkDir, tmeta.len(), modified, target),
            Ok(tmeta) if tmeta.is_file() => (EntryKind::SymlinkFile, tmeta.len(), modified, target),
            Ok(_) => (EntryKind::Other, 0, modified, target),
            Err(_) => (EntryKind::SymlinkBroken, 0, modified, target),
        };
    }
    if lmeta.is_dir() {
        (EntryKind::Dir, 0, modified, None)
    } else if lmeta.is_file() {
        (EntryKind::File, lmeta.len(), modified, None)
    } else {
        (EntryKind::Other, lmeta.len(), modified, None)
    }
}
...
pub fn read_directory(dir: &Path, show_hidden: bool) -> Result<Vec<Entry>, FsError> {
    ...
    for item in read {
        ...
        let (kind, size, modified, symlink_target) = classify(&item.path());
        entries.push(Entry {
            name,
            kind,
            size,
